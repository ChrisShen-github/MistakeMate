...
const standardSettings: PrintSettings = {
  ...
  showAnswers: false, showMeta: true, showDifficulty: true, showDate: false,
  showScoreFields: true, title: 'MistakeMate 错题练习', footer: '由 MistakeMate 整理',
}
...
function estimateQuestionHeight(question: PrintableQuestion) {
  const contentWidth = Math.max(35, (paperSize.value.width - settings.value.margins.left - settings.value.margins.right - (settings.value.columns - 1) * 7) / settings.value.columns)
    ...
    for (const part of question.parts) {
      ...
      if (part.part_type !== '题组说明') {
        height += settings.value.showAnswers
          ...
          : partAnswerLines(question, part) * 7
      }
    }
  ...
  else height += questionAnswerLines(question) * 7
  return height + settings.value.questionGap
}

const pages = computed<PrintPage[]>(() => {
  if (!selectedQuestions.value.length) return []
  const headerHeight = settings.value.showScoreFields ? 29 : 20
  const availableHeight = Math.max(40, paperSize.value.height - settings.value.margins.top - settings.value.margins.bottom - headerHeight - 9)
    ...
    <div v-else class="studio-grid">
        ...
        <div class="question-editor-list">
          <article v-for="question in visibleQuestions" :key="question.id" class="question-editor" :class="{ selected: selectedIds.includes(question.id) }">
            ...
            <div v-if="selectedIds.includes(question.id)" class="question-tools">
              ...
              <div v-if="!question.parts.length && !settings.showAnswers" class="tool-row">
                <span>答题行</span>
              ...
              <div v-if="question.parts.length && !settings.showAnswers" class="part-space-list">
      ...
      <main class="preview-panel" :class="{ 'mobile-visible': mobileStep === 'preview' }" aria-label="打印分页预览">
        <div class="preview-toolbar screen-only">
          <div><strong>{{ paperPresets.find((paper) => paper.id === settings.paperId)?.label.split('（')[0] }}</strong><span>{{ settings.orientation === 'portrait' ? '竖版' : '横版' }} · {{ settings.columns }} 栏 · {{ pages.length }} 页</span></div>
        ...
        <div v-else class="page-stage">
          <div class="page-stack" :style="paperStyle">
            <article v-for="(page, pageIndex) in pages" :key="pageIndex" class="paper-sheet">
              ...
              <div class="paper-body">
                <div v-for="(column, columnIndex) in page.columns" :key="columnIndex" class="paper-column">
                  <section v-for="item in column" :key="item.question.id" class="print-question" :class="{ oversized: item.oversized }">
                    ...
                    <div class="question-content">
                      ...
                      <div v-if="item.question.parts.length" class="part-list">
                        <section v-for="part in rootParts(item.question)" :key="part.id" class="question-part">
                          ...
                          <template v-if="part.part_type !== '题组说明'">
                            ...
                            <div v-else class="answer-lines"><span v-for="line in partAnswerLines(item.question, part)" :key="line"></span></div>
                          </template>
                          <section v-for="child in childParts(item.question, part.id)" :key="child.id" class="question-part child">
                            ...
                            <div v-else class="answer-lines"><span v-for="line in partAnswerLines(item.question, child)" :key="line"></span></div>
                          </section>
                        </section>
                      </div>
                      ...
                      <div v-else class="answer-lines"><span v-for="line in questionAnswerLines(item.question)" :key="line"></span></div>
                      <p v-if="item.oversized" class="overflow-warning screen-only">本题超过单页高度，请减少答题行、字号或页边距。</p>
                    </div>
                  </section>
                </div>
              </div>
              <footer class="paper-footer"><span>{{ settings.footer }}</span><span>第 {{ pageIndex + 1 }} / {{ pages.length }} 页</span></footer>
            </article>
          </div>
        </div>
      </main>
      ...
      <aside class="studio-panel settings-panel screen-only" :class="{ 'mobile-visible': mobileStep === 'settings' }" aria-label="模板和版式设置">
        ...
        <details open>
          ...
          <div class="settings-body">
            <label>纸张大小<select v-model="settings.paperId"><option v-for="paper in paperPresets" :key="paper.id" :value="paper.id">{{ paper.label }}</option></select></label>
            ...
            <fieldset><legend>方向</legend><label><input v-model="settings.orientation" type="radio" value="portrait" />竖版</label><label><input v-model="settings.orientation" type="radio" value="landscape" />横版</label></fieldset>
        ...
        <details>
          <summary>页眉信息</summary>
          <div class="settings-body checkbox-list"><label><input v-model="settings.showScoreFields" type="checkbox" />姓名、日期和得分栏</label><label><input v-model="settings.showMeta" type="checkbox" />学科和题型</label><label><input v-model="settings.showDifficulty" type="checkbox" />难度星级</label><label><input v-model="settings.showDate" type="checkbox" />收录日期</label></div>
        </details>
