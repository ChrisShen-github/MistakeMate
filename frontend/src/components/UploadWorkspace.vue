...
const uploadGroups = computed(() => {
  ...
  files.value.forEach((_, index) => {
    group.push(index)
    if (!mergeLinks.value[index]) { groups.push(group); group = [] }
  })
  return groups
})
...
function addFiles(incoming: FileList | File[]) {
  ...
  mergeLinks.value = Array.from({ length: Math.max(0, files.value.length - 1) }, (_, index) => mergeLinks.value[index] ?? false)
...
function removeFile(index: number) { files.value.splice(index, 1); cropRegions.value.splice(index, 1); mergeLinks.value = Array(Math.max(0, files.value.length - 1)).fill(false); errorMessage.value = '' }
function toggleMergeWithNext(index: number) { mergeLinks.value[index] = !mergeLinks.value[index] }
...
<template>
  <section class="upload-page" aria-labelledby="upload-heading">
    ...
    <header class="upload-intro"><p class="eyebrow">添加到{{ props.notebookName }}</p><h1 id="upload-heading">上传错题</h1><p>上传试卷、作业或错题照片。默认每张图片生成一道题；跨页题目可将相邻图片合并后再识别。</p></header>
...
.merge-row { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin: -2px 8px -2px 43px; padding: 8px 0 8px 13px; color: #72849a; border-left: 2px dashed #c6d5e4; font-size: 11px; line-height: 1.45; }.merge-row.active { color: #356c5b; border-color: #70bc9c; }.merge-row button { display: inline-flex; min-height: 40px; flex: 0 0 auto; align-items: center; justify-content: center; gap: 5px; padding: 7px 9px; color: #315f9b; border: 1px solid #bad0ec; border-radius: 8px; background: #f7fbff; font-size: 11px; font-weight: 700; cursor: pointer; transition: background .18s ease,border-color .18s ease; }.merge-row.active button { color: #287158; border-color: #9dceb9; background: #effaf4; }.merge-row button:hover { border-color: #7da7da; background: #edf5ff; }.merge-row button:focus-visible { outline: 3px solid rgba(37,99,235,.2); outline-offset: 2px; }@media (max-width: 760px) { .merge-row { align-items: stretch; flex-direction: column; margin-left: 18px; padding: 10px 0 10px 12px; }.merge-row button { width: 100%; min-height: 44px; } }
</style>
