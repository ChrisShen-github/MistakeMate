...
import { ArrowLeft, BadgeCheck, CheckCircle2, Crop, Eraser, FileImage, FileText, ImagePlus, Link2, LoaderCircle, LockKeyhole, ScanText, Sparkles, Trash2, Unlink, UploadCloud } from '@lucide/vue'
import ImageCropper, { type CropRegion } from './ImageCropper.vue'

const props = defineProps<{ notebookName: string }>()
const emit = defineEmits<{ back: []; queued: [count: number, workflow: 'text' | 'clean' | 'image']; 'configure-ai': [] }>()
...
function defaultImageTitle() {
  const now = new Date()
  const date = new Intl.DateTimeFormat('zh-CN', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', hour12: false }).format(now).replace(/\//g, '-').replace(',', '')
  return `题图 · ${date}`
}
const title = ref(defaultImageTitle())
const note = ref('')
const workflow = ref<'text' | 'clean' | 'image'>('text')
const recognitionMode = ref<'local' | 'ai'>('ai')
const aiConfigured = ref(false)
const cleanConfigured = ref(false)
const defaultWorkflow = ref<'ask' | 'text' | 'clean'>('ask')
const canSubmit = computed(() => files.value.length > 0 && !isSubmitting.value && (workflow.value === 'clean' ? cleanConfigured.value : workflow.value === 'image' || recognitionMode.value === 'local' || aiConfigured.value))
...
const questionCount = computed(() => workflow.value === 'image' ? files.value.length : uploadGroups.value.length)
...
function selectWorkflow(value: 'text' | 'clean' | 'image') {
  workflow.value = value
  if (value === 'image') { mergedGroups.value = []; cancelMergeSelection() }
  if (value !== 'text' && !title.value.trim()) title.value = defaultImageTitle()
  errorMessage.value = ''
}
async function loadAiAvailability() {
  try {
    ...
    if (preferenceResponse.ok) {
      ...
      if (payload.default_upload_workflow === 'text' || payload.default_upload_workflow === 'clean') selectWorkflow(payload.default_upload_workflow)
    }
...
async function queueRecognition() {
  ...
  if (workflow.value !== 'text') formData.set('title', title.value.trim())
  ...
  formData.set('merge_groups', JSON.stringify(workflow.value === 'image' ? files.value.map((_, index) => [index]) : uploadGroups.value))
  ...
  try {
    ...
    mergedGroups.value = []
    cancelMergeSelection()
    emit('queued', count, workflow.value)
  } catch (error) {
    ...
    <div class="upload-layout">
      <section class="upload-card" aria-labelledby="file-heading">
        ...
        <div v-if="files.length" class="file-list" aria-label="待识别文件">
          <div class="file-list-head">
            ...
            <button v-if="!isSelectingMerge && workflow !== 'image'" class="merge-mode-button" type="button" :disabled="files.length < 2" @click="startMergeSelection"><Link2 :size="16" />合并图片</button>
            <button v-else-if="isSelectingMerge" class="cancel-merge-button" type="button" @click="cancelMergeSelection">取消</button>
          </div>
          <template v-for="(file, index) in files" :key="`${file.name}-${file.size}`">
            ...
            <article class="file-row" :class="{ selected: selectedMergeIndexes.includes(index), grouped: Boolean(mergeGroupFor(index)) }">
              ...
              <div class="file-info"><strong>{{ file.name }}</strong><span>{{ formatSize(file.size) }} · {{ cropRegions[index] ? '已裁剪，将保存为最终题图' : workflow === 'image' ? '直接保存为题图' : '使用整张图片' }}</span></div>
              <button v-if="canCrop(file)" class="crop-button" type="button" :aria-label="`截取 ${file.name} 的题目范围`" @click="openCrop(index)"><Crop :size="16" />{{ cropRegions[index] ? '调整题图' : '截取题图' }}</button><button class="remove-button" type="button" :aria-label="`移除 ${file.name}`" @click="removeFile(index)"><Trash2 :size="17" /></button>
            </article>
          </template>
      ...
      <aside class="details-column">
        <section class="details-card" aria-labelledby="details-heading">
          <div class="section-title"><div><p class="section-kicker">第二步</p><h2 id="details-heading">补充一点信息</h2></div></div>
          <p class="details-tip">文字识别的题目可在识别后补充信息；保存图片题目时可先给它一个标题。</p>
          <div class="form-grid"><label>学科<select v-model="subject"><option>数学</option><option>语文</option><option>英语</option><option>其他</option></select></label><label>题目来源<select v-model="source"><option>作业</option><option>试卷</option><option>练习册</option><option>其他</option></select></label></div>
          <label v-if="workflow !== 'text'" class="title-label">错题标题 <span>可修改，默认按上传时间命名</span><input v-model="title" maxlength="120" /></label>
          <label class="note-label">给自己留个备注 <span>可选</span><textarea v-model="note" placeholder="例如：第 2 单元周测，孩子说这题当时没看懂。"></textarea></label>
          <fieldset class="recognition-mode">
            <legend>本次处理流程 <small v-if="defaultWorkflow !== 'ask'">已按偏好带入</small></legend>
            <button type="button" class="mode-choice" :class="{ active: workflow === 'text' }" :aria-pressed="workflow === 'text'" @click="selectWorkflow('text')"><FileText :size="18" /><span><strong>识别为可编辑题目</strong><small>适合知识点分类、编辑题干、组卷和针对性练习</small></span></button>
            ...
            <button type="button" class="mode-choice" :class="{ active: workflow === 'image' }" :aria-pressed="workflow === 'image'" @click="selectWorkflow('image')"><BadgeCheck :size="18" /><span><strong>直接保存题图</strong><small>适合已在外部裁好、去除笔迹的图片；不调用 OCR 或 AI</small></span></button>
            <p v-if="workflow === 'clean'" class="mode-tip" :class="{ warning: !cleanConfigured }">{{ cleanConfigured ? '将调用图片修复模型生成清洁图；这次不会生成可编辑文字，之后仍可手动发起文字识别。' : '尚未配置图片修复模型，请先在 AI 设置中选择 image 模型。' }}<button v-if="!cleanConfigured" type="button" @click="emit('configure-ai')">去配置 AI</button></p>
            <p v-else-if="workflow === 'image'" class="mode-tip">每张图片会直接成为一题可打印的原题图，不会发送到 OCR 或 AI 服务。</p>
          </fieldset>
          <fieldset v-if="workflow === 'text'" class="recognition-mode nested"><legend>文字识别方式</legend><button type="button" class="mode-choice" :class="{ active: recognitionMode === 'local' }" :aria-pressed="recognitionMode === 'local'" @click="selectRecognitionMode('local')"><ScanText :size="18" /><span><strong>本地 OCR</strong><small>免费、题图不离开设备</small></span></button><button type="button" class="mode-choice" :class="{ active: recognitionMode === 'ai' }" :aria-pressed="recognitionMode === 'ai'" @click="selectRecognitionMode('ai')"><Sparkles :size="18" /><span><strong>AI 视觉识别</strong><small>{{ aiConfigured ? '跳过本地 OCR，直接使用视觉模型' : '需要先完成 AI 设置' }}</small></span></button><p v-if="recognitionMode === 'ai'" class="mode-tip" :class="{ warning: !aiConfigured }">{{ aiConfigured ? '图片会发送给你配置的 AI 服务，适合手写、公式和复杂版面。' : '尚未配置视觉模型，请先完成 AI 设置后再上传。' }}<button v-if="!aiConfigured" type="button" @click="emit('configure-ai')">去配置 AI</button></p></fieldset>
        </section>
        <section class="privacy-note"><LockKeyhole :size="19" /><div><strong>题目只用于生成你的错题本</strong><p>选择裁剪后，裁剪图会作为最终题图保存；AI 识别结果可随时修改或删除。</p></div></section>
      </aside>
    </div>
    <footer class="upload-footer"><p><CheckCircle2 :size="17" />{{ workflow === 'clean' ? `将为 ${files.length} 张图片生成清洁打印图，并以标题“${title || '上传时间'}”归档。` : workflow === 'image' ? `将把 ${files.length} 张图片直接保存为可打印题图，不进行识别。` : `将由 ${files.length} 张图片生成 ${questionCount} 道题；合并的图片会一起识别并确认。` }}</p><button class="recognize-button" type="button" :disabled="!canSubmit" @click="queueRecognition"><LoaderCircle v-if="isSubmitting" class="spin" :size="18" />{{ isSubmitting ? '正在上传…' : workflow === 'clean' ? (cleanConfigured ? `去除 ${files.length} 张图的笔迹` : '请先配置图片修复模型') : workflow === 'image' ? `直接保存 ${questionCount} 道题` : recognitionMode === 'ai' ? (aiConfigured ? `使用 AI 识别 ${questionCount} 道题` : '请先配置 AI') : `生成 ${questionCount} 道题并识别` }}</button></footer>
...
.title-label { display: grid; gap: 7px; margin-top: 15px; color: #405a75; font-size: 12px; font-weight: 700; }.title-label span { color: #8596a8; font-size: 11px; font-weight: 500; }.title-label input { width: 100%; min-height: 44px; padding: 0 11px; color: #2d4662; border: 1px solid #cbd8e6; border-radius: 8px; background: #fff; font: inherit; font-size: 14px; }.title-label input:focus { border-color: #2563eb; outline: 3px solid rgba(37,99,235,.16); }
