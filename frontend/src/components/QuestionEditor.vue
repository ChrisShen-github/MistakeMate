...
import { ArrowLeft, CheckCircle2, CircleAlert, FileText, ImagePlus, Layers3, LoaderCircle, Plus, Save, Sparkles, Trash2 } from '@lucide/vue'
...
const props = defineProps<{ batchId: string; question: MistakeQuestion; canAddFigure?: boolean }>()
const emit = defineEmits<{ saved: [question: MistakeQuestion]; finished: []; 'add-figure': []; 'remove-figure': [figureId: string] }>()
...
<template>
  <article class="question-editor" :aria-labelledby="`question-${question.id}`">
    ...
    <section class="figure-section" aria-label="题图">
      <div><strong>题图</strong><small>坐标轴、几何图、统计图等可从原图单独裁出，确认后会随题目打印。</small></div>
      <button type="button" :disabled="!canAddFigure" @click="emit('add-figure')"><ImagePlus :size="17" />从原图截取题图</button>
