...
const useManualImageEditModel = ref(false)
...
const imageModels = computed(() => availableModels.value.filter((item) => item.toLowerCase().includes('image')))
...
async function loadModels() {
  ...
  try {
    ...
    useManualImageEditModel.value = false
...
<template>
  <section class="ai-settings-page" aria-labelledby="ai-settings-title">
    ...
    <form v-else class="ai-card" @submit.prevent="saveConfig">
      ...
      <section class="image-edit-section" aria-labelledby="image-edit-heading"><div class="section-top"><div><h3 id="image-edit-heading">图片修复模型 <span>可选</span></h3><p>用于后续去除笔迹、生成清洁打印图；不参与文字识别，也不会覆盖原图。</p></div></div><label v-if="imageModels.length && !useManualImageEditModel" for="image-edit-model">选择图片模型<select id="image-edit-model" v-model="imageEditModel"><option value="">暂不启用图片修复</option><option v-for="item in imageModels" :key="item" :value="item">{{ item }}</option></select><ChevronDown class="select-icon image-select-icon" :size="18" /></label><label v-else for="image-edit-model">图片编辑模型 ID<input id="image-edit-model" v-model.trim="imageEditModel" placeholder="例如 gpt-image-1；留空则暂不启用" /><small>仅填写支持 OpenAI Images Edit 的图片模型 ID。和上方共用接口地址与 API 密钥。</small></label><button class="text-button" type="button" @click="useManualImageEditModel = !useManualImageEditModel">{{ useManualImageEditModel ? '返回图片模型列表选择' : '列表没有图片模型？手动填写模型 ID' }}</button><p class="image-edit-note"><ShieldCheck :size="17" /><span>清洁图会作为单独版本保存，生成后需由你确认是否用于打印。</span></p></section>
...
<style scoped>
.image-edit-section{margin-top:16px;padding:19px;border:1px solid #ded5f7;border-radius:12px;background:#fcfbff}.image-edit-section h3{margin:0;color:#4f3d83;font-size:15px}.image-edit-section h3 span{margin-left:5px;color:#7b6aa7;font-size:11px;font-weight:600}.image-edit-section label{position:relative;display:grid;gap:7px;color:#4c4663;font-size:13px;font-weight:700}.image-edit-section input,.image-edit-section select{width:100%;min-height:46px;padding:0 12px;color:#2b435d;border:1px solid #d3c9ed;border-radius:9px;background:#fff;font:inherit}.image-edit-section select{appearance:none;padding-right:40px}.image-edit-section input:focus,.image-edit-section select:focus{outline:0;border-color:#795dd1;box-shadow:0 0 0 3px rgba(121,93,209,.12)}.image-select-icon{right:12px;bottom:14px;color:#7053c8}.image-edit-note{display:flex;align-items:flex-start;gap:8px;margin:13px 0 0;padding:10px 11px;color:#5f5b74;border-radius:8px;background:#f4f1fd;font-size:12px;line-height:1.55}.image-edit-note svg{flex:0 0 auto;margin-top:1px;color:#7053c8}
