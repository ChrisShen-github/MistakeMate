...
type BatchDetail = { id: string; subject: string; source: string; note: string; status: string; created_at: string; file_count: number; files: UploadedFile[]; ocr: OcrRun | null }
...
function ocrLabel(status?: string) { return ({ queued: '等待识别', running: '正在识别', completed: '识别完成', failed: '识别失败' } as Record<string, string>)[status ?? ''] ?? '尚未识别' }
...
<template>
  <section class="review-page" aria-labelledby="review-heading">
    ...
    <template v-else-if="batch">
      ...
      <section v-if="batch.ocr?.status === 'completed'" class="ocr-result" aria-labelledby="ocr-result-heading"><div class="section-heading"><div><p class="eyebrow">OCR 初稿</p><h2 id="ocr-result-heading">识别出的文字</h2></div><span>{{ batch.ocr.engine }}</span></div><pre>{{ batch.ocr.text || '没有识别出可编辑文字，请检查图片清晰度后重试。' }}</pre></section>
