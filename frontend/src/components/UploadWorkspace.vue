...
import { ArrowLeft, CheckCircle2, FileImage, FileText, ImagePlus, LoaderCircle, LockKeyhole, Trash2, UploadCloud } from '@lucide/vue'
...
function addFiles(incoming: FileList | File[]) {
  errorMessage.value = ''
  const validFiles = Array.from(incoming).filter((file) => file.type.startsWith('image/') || file.type === 'application/pdf').filter((file) => file.size <= 20 * 1024 * 1024)
    ...
    <div class="upload-layout">
      <section class="upload-card" aria-labelledby="file-heading">
        ...
        <div class="drop-zone" :class="{ dragging: isDragging }" aria-describedby="upload-tip upload-error" @dragenter.prevent="isDragging = true" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false" @drop.prevent="onDrop">
          <div class="drop-icon"><UploadCloud :size="27" /></div><strong>拖入图片，或点这里选择文件</strong><span id="upload-tip">支持 JPG、PNG、HEIC、PDF；单个文件最大 20 MB</span><button class="choose-button" type="button" @click.stop="chooseFiles"><ImagePlus :size="17" />选择文件</button><input ref="input" class="sr-only" type="file" accept="image/*,.pdf" multiple @change="onInputChange" />
        </div>
