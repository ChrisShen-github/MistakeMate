...
function statusLabel(status: string) {
  return status === 'queued' ? '等待识别' : status
}
    ...
    <section class="library-list" aria-live="polite" :aria-busy="isLoading">
      ...
      <article v-for="batch in batches" v-else :key="batch.id" class="batch-card">
        ...
        <div class="batch-main">
          <div class="batch-title"><span class="subject-chip">{{ batch.subject }}</span><strong>{{ batch.source }}错题</strong><span class="status-chip">{{ statusLabel(batch.status) }}</span></div>
